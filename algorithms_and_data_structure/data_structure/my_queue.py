...
class MyQueue:
    """
    Class implements queue structure
    """
    def __init__(self, value=None):
        if value:
            self.length = 1
...
if __name__ == '__main__':
    my_list = MyQueue("zero node")
    print(my_list.peek)
    my_list.enqueue("zero node")
    my_list.enqueue("Second node")
    my_list.enqueue("Third node")
    my_list.enqueue("Fourth node")
    my_list.enqueue("Fifth node")
    my_list.enqueue("Sixth node")
    print(my_list.peek)
    my_list.show()
